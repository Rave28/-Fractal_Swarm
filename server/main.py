...
# We assume this file lives in <SWARM_ROOT>/server/main.py
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
SWARM_ROOT = os.path.abspath(os.path.join(CURRENT_DIR, ".."))
# Use a unified DB path, or fall back to user home
DB_ROOT = os.path.join(SWARM_ROOT, "core", "brain", "myBrAIn", "chroma_db")
if not os.path.exists(DB_ROOT):
    DB_ROOT = os.path.expanduser("~/mybrain_data")
...
app = FastAPI(
    ...
    description="FastAPI + UV + Distributed Swarm Intelligence",
    version="1.2.0",
)
...
class VectorSearchEngine:
    async def semantic_search(self, query: str, limit: int) -> List[SearchResult]:
        ...
        # Dynamically locate the search wrapper relative to this file
        wrapper_path = os.path.join(CURRENT_DIR, "search_wrapper.py")

        # We assume the environment is managed by UV in the server dir
        cmd = [
            "python",
            wrapper_path,
            query,
...
async def trigger_research(request: ResearchRequest) -> ResearchResponse:
    ...
    print(
        "⚠️ CACHE MISS: Novel concept detected. Routing to Perplexity Sonar-Pro...",
        file=sys.stderr,
    )
...
# --- A2A Protocol (Inter-Swarm Communication) ---
...
class A2AExchangeRequest(BaseModel):
    session_token: str
    semantic_payload: str
    source_context: str


class A2AExchangeResponse(BaseModel):
    status: str
    vector_id: str
    tokens_processed: int


@app.post("/a2a/handshake", response_model=A2AHandshakeResponse, tags=["A2A Protocol"])
async def a2a_handshake(request: A2AHandshakeRequest):
    """
    Authenticate a foreign Swarm node to establish a vector trading session.
    """
    session_token = f"a2a_sess_{uuid.uuid4().hex}"
    print(
        f"🌍 [A2A] Handshake successful with foreign agent: {request.agent_id}",
        file=sys.stderr,
    )
    return A2AHandshakeResponse(
        status="authenticated",
        session_token=session_token,
        supported_protocols=["vibe_vector_sync_v1", "auto_browse_stream_v2"],
    )


@app.post("/a2a/exchange", response_model=A2AExchangeResponse, tags=["A2A Protocol"])
async def a2a_exchange(request: A2AExchangeRequest):
    """
    Accept an inbound semantic payload from a foreign Swarm and inject it into local MyBrain memory.
    """
    if not request.session_token.startswith("a2a_sess_"):
        raise HTTPException(status_code=401, detail="Invalid A2A session token")

    print(
        f"🌍 [A2A] Receiving inbound vector payload from source: {request.source_context}",
        file=sys.stderr,
    )

    doc_id = f"a2a_mem_{uuid.uuid4().hex[:8]}"

    try:
        if collection:
            collection.upsert(
                ids=[doc_id],
                documents=[request.semantic_payload],
                metadatas=[
                    {"source": request.source_context, "agent": "Foreign_A2A_Node"}
                ],
            )
        else:
            raise HTTPException(status_code=503, detail="MyBrain offline")
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"A2A Memory Injection failed: {str(e)}"
        )

    return A2AExchangeResponse(
        status="synchronized",
        vector_id=doc_id,
        tokens_processed=len(request.semantic_payload.split()),
    )
